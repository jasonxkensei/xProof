...
- **Agent observability**: `GET /api/me` endpoint for API key holders returns key info, trial quota, trial_used, trial_remaining, and total certifications. `GET /api/certifications` now accepts Bearer API key (previously wallet-only). Successful `POST /api/proof` and `POST /api/batch` responses include `X-Trial-Remaining` header — agents can track quota without extra calls. Both `llms.txt` and `llms-full.txt` updated with new endpoint docs.
- **Pricing endpoint unified**: `/api/certification-price` → 301 redirect to `/api/pricing`. `/api/pricing` now includes `price_egld`, `egld_usd_rate`, `receiver_address`, and admin-wallet zero-price logic. Frontend `certify.tsx` updated to use new endpoint.
