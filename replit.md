...
A public `/docs` page provides a comprehensive, searchable API reference with collapsible endpoint groups covering all API sections (Core, Trust & Leaderboard, Attestations, Agent Protocols, Discovery, Webhooks, Credits & Payments). Includes curl examples with copy buttons, authentication overview, and method badges. Linked from the landing page header and footer. Integration guides: `/docs/4w` (4W Certification Workflow — WHO/WHAT/WHEN/WHY for full agent auditability) and `/docs/trading` (Proof of Trade Execution — async, non-blocking pattern for trading agents with real-world latency data).
