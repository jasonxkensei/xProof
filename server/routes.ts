...
`;
  ...
  app.get("/api/agents/:wallet/incident-report", publicReadRateLimiter, async (req, res) => {
    try {
      ...
      if (!uuidRegex.test(proofId)) {
        return res.status(400).json({ error: "Invalid proof_id format — expected UUID" });
      }
      ...
      if (!user || !user.isPublicProfile) {
        return res.status(404).json({ error: "Agent profile not found or not public" });
      }
      ...
      if (!contestedProof) {
        return res.status(404).json({ error: "Proof not found for this agent" });
      }
  ...
  // GET /api/attestations/expiring — list attestations expiring within N days (issuer auth)
  app.get("/api/attestations/expiring", isWalletAuthenticated, async (req: any, res) => {
    try {
      const issuerWallet = req.walletAddress;
      const days = Math.min(Math.max(parseInt(req.query.days as string || "30"), 1), 90);
